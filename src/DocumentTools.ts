...
export async function executeToolCall(
  ...
  if (name === 'append_note') {
    ...
    new Notice('Threads Orchestrator: appended note');
  ...
  if (name === 'insert_at_cursor') {
    ...
    new Notice('Threads Orchestrator: inserted text at cursor');
  ...
  if (name === 'replace_document') {
    ...
    new Notice('Threads Orchestrator: replaced document content');
  ...
  if (name === 'open_file') {
    ...
    try {
      ...
      if (existingLeaf) {
        ...
        new Notice(`Threads Orchestrator: switched to ${file.name}`);
        return `Switched to already-open ${file.path}`;
      }
      const leaf = app.workspace.getLeaf('tab');
      await leaf.openFile(file);
      new Notice(`Threads Orchestrator: opened ${file.name}`);
  ...
  if (name === 'create_document') {
    ...
    try {
      ...
      new Notice(`Threads Orchestrator: created ${file.name}`);
